            ...
            {report && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm overflow-y-auto" onClick={onClose}>
                    <motion.div
                        ...
                        className="bg-white rounded-2xl shadow-soft-lg w-full max-w-2xl my-8"
                    ...
                    >
                        ...
                        <div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto">
                            {/* Risk Level */}
                            <div className={`flex items-center gap-3 p-4 rounded-xl border ${report.risk_level === 'GREEN' ? 'bg-emerald-50 border-emerald-100 text-emerald-700' :
                                report.risk_level === 'YELLOW' ? 'bg-amber-50 border-amber-100 text-amber-700' :
                            ...
                            {report.analysis_result && report.analysis_result.length > 0 && (
                                <div>
                                    <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3">Extracted Values</h3>
                                    <div className="grid sm:grid-cols-2 gap-3">
                                        {report.analysis_result.map((finding, idx) => {
                                            const statusColor =
                                                finding.status?.toLowerCase().includes('normal') ? 'bg-emerald-50 border-emerald-100 text-emerald-700' :
                                                    finding.status?.toLowerCase().includes('high') ? 'bg-red-50 border-red-100 text-red-700' :
                                                        finding.status?.toLowerCase().includes('low') ? 'bg-amber-50 border-amber-100 text-amber-700' :
                                            ...
                                            return (
                                                <div key={idx} className={`p-4 rounded-xl border ${statusColor}`}>
                                                    <div className="flex items-center justify-between mb-1">
                                                        ...
                                                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${finding.status?.toLowerCase().includes('normal') ? 'bg-emerald-100 text-emerald-700' :
                                                            finding.status?.toLowerCase().includes('high') ? 'bg-red-100 text-red-700' :
                                                                finding.status?.toLowerCase().includes('low') ? 'bg-amber-100 text-amber-700' :
                            ...
                            {report.key_findings && report.key_findings.length > 0 && (
                                <div>
                                    <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3">Key Findings</h3>
                                    <ul className="space-y-2">
                                        {report.key_findings.map((finding, idx) => (
                                            <li key={idx} className="flex items-start gap-3 p-3 bg-slate-50 rounded-xl border border-slate-100">
                                                <div className="w-5 h-5 bg-primary-100 text-primary rounded-full flex items-center justify-center shrink-0 mt-0.5 text-xs font-bold">
                                                    {idx + 1}
                                                </div>
                                ...
                                <div>
                                    ...
                                    <ul className="space-y-2">
                                        {report.recommendations.map((rec, idx) => (
                                            <li key={idx} className="flex items-start gap-3 p-3 bg-primary-50/50 rounded-xl border border-primary-100">
                        ...
                        {/* Footer */}
                        <div className="flex items-center justify-between gap-4 p-6 border-t border-slate-100 bg-slate-50/50 rounded-b-2xl">
                            <a
